...
def calculate_optimal_weights(stock_names, stock_predictions, benchmark_returns):
    ...
    # Calculate the portfolio's performance relative to the benchmark
    portfolio_returns = df.pct_change().dropna()
    portfolio_returns.rename(
        columns={'close': 'Adj Close'}, inplace=True, level=0)
    ...
    return filtered_weights, portfolio_performance
